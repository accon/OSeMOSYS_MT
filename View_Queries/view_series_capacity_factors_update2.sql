...
--     JOIN scenario_area sca ON m3.scenario_area_fk = sca.id
	JOIN mapping_scenario_series_capacity_factor m4 ON scf.id = m4.series_capacity_factor_fk
	JOIN scenario_series_capacity_factor scscf ON m4.scenario_series_capacity_factor_fk = scscf.id
     JOIN modelling_year my ON scf.modelling_year_fk = my.id,
    modelling_run_properties mrp,
    choice_modelling_run_properties cmrp
  WHERE scscf.id = mrp.scenario_series_capacity_factor_fk AND mrp.id = cmrp.modelling_run_properties_fk
