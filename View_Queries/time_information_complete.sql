CREATE OR REPLACE VIEW time_information_complete AS 
 SELECT DISTINCT ts.pk AS timeslice_id,
    ts.name AS timeslice_name,
    ts.hours_per_timeslice,
    s.season_pk AS season_id,
   ...
   FROM timeslice ts
     JOIN mapping_hourly_time_blocks_timeslices m ON m.starting_timeslice_fk = ts.pk
     JOIN season s ON ts.season_fk = s.season_pk
     JOIN daytype dt ON ts.daytype_fk = dt.pk
     JOIN daylitimebracket dtb ON ts.dailytimebracket_fk = dtb.pk
  ...
  WHERE st.id = mrp.scenario_time_fk AND mrp.scenario_time_fk = cmrp.modelling_run_properties_fk
