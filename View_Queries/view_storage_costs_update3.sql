-- View: view_storage_costs

DROP VIEW view_storage_costs;

CREATE OR REPLACE VIEW view_storage_costs AS 
 SELECT stc.id AS storage_cost_id,
    stc.capital_cost__k_euro_mwh,
    stc.fix_cost__k_euro_mw_a,
    stc.variable_cost__k_euro_mwh,
   ...
   FROM storage_cost stc
     ...
     JOIN mapping_scenario_storage_cost m ON m.storage_cost_fk = stc.id
