...

{
  ...
  line_switch <- length(read.table(file=filename, header=FALSE, 
                                   skip = line_actual-1, 
                                   nrow=1, fill=F)) > 5 ||      # 5 columns indicate that the line includes a semicolon
                (read.table(file=filename, header=FALSE, skip = line_actual-1, 
                                  nrow=1, fill=F))[1,1] == "set"
  ...
  if (missing(no_rows))
  {
    if(missing(end_pattern))
    {
      ...
      data_uncut <- read.table(file=filename, header=log_head, 
                               skip = line_actual, 
                               nrow=(line_next-line_actual), fill=T) 
      #, blank.lines.skip=F
      
      #The following is neccessary because read.table automatically skips comment and blank lines
      data <- data_uncut[-c(min(grep("\\[", data_uncut[,1])[which(grep("\\[", data_uncut[,1])>1)], # [which(...)] makes sure that first line "\\[" isn't taken into account
                                grep("param",data_uncut[,1])[which(grep("param", data_uncut[,1])>1)], # [which(...)] makes sure that first line "param" isn't taken into account
                                grep(";", data_uncut[,1]), na.rm=TRUE):nrow(data_uncut)),]
    }
    else
    {
