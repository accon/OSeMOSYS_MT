data_extraction_output <- function(filename, pattern, no_rows, end_pattern, log_head=FALSE)
  
{
  ...
  if (missing(no_rows))
  {
    ...
    else
    {
      ...
      no_tech <- length(which(data_raw[,1] == "")) # Is maybe also possible in a more elegant way
      vec_data_id <- as.character(data_raw[c(seq(from=1, to=nrow(data_raw)-no_timeslices-1, 
      ...
      vec_split <- rep(c(1:no_tech), each=no_timeslices+2)
      mat_split <- matrix(vec_split, nrow=(no_timeslices+2)*no_tech, ncol=no_years)
