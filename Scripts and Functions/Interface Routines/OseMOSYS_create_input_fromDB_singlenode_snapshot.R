...
filename <- "OSeMOSYS_input_MR1_1_12days.txt"
...
#-------Demand--------ToDo-arrange TimeSlice TimeStamp association in DB-----------------------------------
...
timeslices_ord <- timeslices[c(grep("Wi", timeslices), grep("In", timeslices), grep("Su", timeslices), 
                             grep("Ra", timeslices))]
#timeslices_order <- timeslices[c(73:144,145:216,1:72,217:288)]
arr_demand <- array(NA, dim=vec_dim_demand, 
                              dimnames=list(timeslices_ord, demand_years, demand_regions))
...
write_matrix_to_file(mat_csp_inputactivityratio, csp_pps, years, 
                     string_def=iau_string_def_csp, filename=filename, multi=TRUE)
...
write_matrix_to_file(mat_conls, timeslices_ord, as.character(c(1:4)), string_init_cls, string_def_cls, 
                     filename)
...
write_matrix_to_file(mat_conld, timeslices_ord, daytypes, string_init_cld, string_def_cld, filename)
...
write_matrix_to_file(mat_conlh, timeslices_ord, dtb, string_init_clh, string_def_clh, 
                     filename)
# DaySplit
