...
# DB extraction of storage cost developments is done in the cost-section but not used there. Instead it
# is used at the end of the document. consolidating the extraction line with the part could be helpful. 
...
source(paste(func_wd, "create_array.R", sep=""))
...
filename <- "OSeMOSYS_input_MR0_4_areatesting.txt"
...
arr_demand <- array(NA, dim=vec_dim_scf, 
...
demand_series_string_init <- "SpecifiedDemandProfile\tdefault\t0\t:="
...
mat_capacity_to_activity_unit <- matrix(vec_capacity_to_activity_unit, ncol=no_power_plant_types, 
                                        nrow=length(regions)) 
string_def_capacity_to_activity_unit <- "param CapacityToActivityUnit\tdefault\t8760\t:" 
write_matrix_to_file(mat_capacity_to_activity_unit, regions, power_plant_types, 
...
mat_flag_peak_pps <- matrix(vec_flag_peak_pps, ncol=no_power_plant_types, nrow=length(regions)) 
string_def_flag_peak_pps <- "param TechWithCapacityNeededToMeetPeakTS\tdefault\t0\t:" 
write_matrix_to_file(mat_flag_peak_pps, regions, power_plant_types, 
...
pp_efficiency_fossil <- pp_efficiency_raw[which(!is.na(pp_efficiency_raw$fuel)),]  # What about Biomass?
power_plants_fossil <- unique(pp_efficiency_fossil$pp_type)  
no_fossil_pp <- length(power_plants_fossil)
iau_regions <- unique(pp_efficiency_fossil$area_aggregated)
no_iau_regions <- length(iau_regions)
iau_years <- unique(pp_efficiency_fossil$modelling_year)
no_iau_years <- length(iau_years)

vec_dim_efficiency <- c(no_fossil_pp, no_iau_years, no_iau_regions)
arr_efficiency <- array(NA, dim=vec_dim_efficiency, 
                    dimnames=list(power_plants_fossil, iau_years, iau_regions))
for (i in dimnames(arr_efficiency)[[3]])
{
  arr_efficiency[,,i] <- matrix(pp_efficiency_fossil$pp_efficiency
                            [which(pp_efficiency_fossil$area_aggregated == i)], 
                            nrow=no_fossil_pp, ncol=no_years, byrow=FALSE)
}

arr_iau <- round(1/arr_efficiency, 2)

iau_string_init <- "param\tInputActivityRatio\tdefault\t0\t:="
iau_string_def <- "[XXX,*,natural_gas,1,*]\t:"

write_array_to_file(arr_iau, power_plants_fossil, iau_years, iau_regions, 
                    iau_string_init, iau_string_def, filename, multi=TRUE)

storage_efficiency_raw <- dbReadTable(db_TN_PSM, "view_storage_efficiency") 

arr_storage_efficiency_charge <- create_array(storage_efficiency_raw, "storage_type", "modelling_year", 
                                       "area_aggregated", "storage_charge_efficiency")

arr_storage_efficiency_discharge <- create_array(storage_efficiency_raw, "storage_type", "modelling_year", 
                                              "area_aggregated", "storage_discharge_efficiency")

arr_iau_st_c <- round(1/arr_storage_efficiency_charge, 2)   #iau = input activity ratio, st = storage, c=charge, dc = discharge
arr_iau_st_dc <- round(1/arr_storage_efficiency_discharge, 2)

iau_st_c_string_init <- ""
iau_st_c_string_def <- "[XXX,*,electricity,2,*]\t:"
iau_st_dc_string_def <- "[XXX,*,electricity,1,*]\t:"

write_array_to_file(arr_iau_st_c, storage_types, years, regions, 
                     string_def=iau_st_c_string_def, filename=filename)
...
# Output Activity Ratio for Power Plant Technologies
vec_dim_oau <- c(length(pp_types_wo_supply), no_years, no_regions)
arr_oau <- array(1, dim=vec_dim_oau, dimnames=list(pp_types_wo_supply, years, regions))

oau_string_init = "param OutputActivityRatio\tdefault\t0\t:="
oau_string_def = paste("[XXX,*,", fuels[2],",1,*]\t:", sep="")
write_array_to_file(arr_oau, pp_types_wo_supply, years, regions, oau_string_init, oau_string_def, 
                    filename)

# Output Activity Ratio for Supply Technologies
vec_dim_oau_supply <- c(length(supply_technologies), no_years, no_regions)
arr_oau_supply <- array(1, dim=vec_dim_oau_supply, dimnames=list(supply_technologies, years, regions))
oau_supply_string_def <- paste("[XXX,*,", fuels[1],",1,*]\t:", sep="")
...
# Should be 1 for power plants/electricity, 1 for supply technologies and their respective fuels and zero for rest."
      , file=filename, append=TRUE)
write_array_to_file(arr_oau, pp_types_wo_supply, years, regions, oau_string_init, 
                     oau_string_def, filename=filename, multi=TRUE)

write_array_to_file(arr_oau_supply, supply_technologies, years, regions, string_def=oau_supply_string_def, 
                     filename=filename)
...
arr_fix_costs_pp <- create_array(power_plant_costs_raw, "power_plant_type", "modelling_year", 
                                 "area_aggregated", "fix_cost__euro_mw", byrow=TRUE) 
                               
fixcost_string_init <- "param FixedCost\tdefault\t0\t:=" 
fixcost_string_def <- "[XXX,*,*]\t:\t"
...
write_array_to_file(arr_fix_costs_pp, power_plant_types, years, regions, fixcost_string_init, 
                     fixcost_string_def, filename)

arr_capital_costs_pp <- create_array(power_plant_costs_raw, "power_plant_type", "modelling_year", 
                                     "area_aggregated", "capital_cost__euro_mw", byrow=TRUE) 
capcost_string_init <- "param CapitalCost\tdefault\t0\t:=" 
capcost_string_def <- "[XXX,*,*]\t:\t"
...
# Total capital cost (including interest paid during construction)per unit of capacity for new capacity additions", 
      file=filename, append=TRUE)	
write_array_to_file(arr_capital_costs_pp, power_plant_types, years, regions, capcost_string_init, 
                     capcost_string_def, filename)


arr_variable_costs_pp <- create_array(power_plant_costs_raw, "power_plant_type", "modelling_year", 
                                      "area_aggregated", "variable_cost__euro_mwh", byrow=TRUE) 
varcost_string_init <- "param VariableCost\tdefault\t0.00001\t:=" 
varcost_string_def <- "[XXX,*,1,*]\t:\t"
...
# This variable records both the nonfuel O&M costs of processes and fuel costs of each fuel supplied to those processes.", 
      file=filename, append=TRUE)	
write_array_to_file(arr_variable_costs_pp, power_plant_types, years, regions, varcost_string_init, 
...
power_plant_list_raw <- dbReadTable(db_TN_PSM, "view_power_plant_list")

pp_types_phaseout <- unique(power_plant_list_raw$power_plant_type)
vec_decommissioning_year <- power_plant_list_raw$commissioning_year + power_plant_list_raw$lifetime__a
power_plants_lifeinfo <- as.data.frame(cbind(power_plant_list_raw$id, power_plant_list_raw$name, power_plant_list_raw$commissioning_year,
                               power_plant_list_raw$lifetime__a, power_plant_list_raw$installed_capacity__mw, 
                               power_plant_list_raw$power_plant_type_fk, power_plant_list_raw$area_aggregated,
                               vec_decommissioning_year)) 
colnames(power_plants_lifeinfo) <- c(colnames(power_plant_list_raw[,c(1,2,8,9,10,12,24)]), "decommissioning_year")
...
vec_dim_phaseout <- c(length(pp_types_phaseout), no_years, no_regions)
arr_phaseout <- array(NA, dim=c(length(pp_types_phaseout), 2, no_regions), 
                      dimnames=list(pp_types_phaseout, c("type_fk", "residual_capacity"), regions))
arr_write_phaseout <- array(NA, dim=vec_dim_phaseout, dimnames=list(pp_types_phaseout, years, regions))

for (i in dimnames(arr_phaseout)[[3]])
{
  for (y in numvec_years)
  {
    mat_phaseout <- phaseout(y, 
                        vec_instcap=power_plants_lifeinfo$installed_capacity__mw[which(power_plants_lifeinfo$area_aggregated == i)], 
                           vec_decom=power_plants_lifeinfo$decommissioning_year[which(power_plants_lifeinfo$area_aggregated == i)], 
                           vec_pptype=power_plants_lifeinfo$power_plant_type_fk[which(power_plants_lifeinfo$area_aggregated == i)])  
    arr_write_phaseout[mat_phaseout$type_fk,which(colnames(arr_write_phaseout) == y),i] <- mat_phaseout$residual_capacity
  }
}

#Replace not available elements (NA) with 0 
arr_write_phaseout[which(is.na(arr_write_phaseout))] <- 0

phaseout_string_init <- "param ResidualCapacity\tdefault\t0\t:=\t"
phaseout_string_def <- "[XXX,*,*]\t:\t"

write_array_to_file(arr_write_phaseout, pp_types_phaseout, years, regions, phaseout_string_init,
...
vec_dim_scf <- c(no_timeslices, no_years, no_tech_scf, no_regions)
arr_capacity_factors <- array(NA, dim=vec_dim_scf, 
                              dimnames=list(timeslices, years, tech_scf, regions))

for (j in dimnames(arr_capacity_factors)[[4]])
{
  for (i in dimnames(arr_capacity_factors)[[3]])
  {
    arr_capacity_factors[,,i,j] <- matrix(series_capacity_factors_raw$available_capacity
                                          [which(series_capacity_factors_raw$power_plant_type == i & 
                                                   series_capacity_factors_raw$area_aggregated == j)], 
                                          nrow=no_timeslices, ncol=no_years, byrow=FALSE)
  }
}
...
# Adding region names to array
vec_dim_scf_array <- c(length(dimnames(arr_capacity_factors)[[1]]), 1,
                       length(dimnames(arr_capacity_factors)[[3]]), length(dimnames(arr_capacity_factors)[[4]]))
vec_region_add <- rep(dimnames(arr_capacity_factors)[[4]], each=length(dimnames(arr_capacity_factors)[[1]])*
                        length(dimnames(arr_capacity_factors)[[3]]))
arr_add_scf <- array(vec_region_add, dim=vec_dim_scf_array)
arr_scf_extended <- abind(arr_capacity_factors, arr_add_scf, along=2) 


write(cfs_string_init,file=filename, append=TRUE)	
# probably a for loop is the easiest way to go
apply(arr_scf_extended, MARGIN=4, 
      function(x) 
      {
        y <- x[,-length(dimnames(x)[[2]]),]
        write_array_to_file(y, timeslices, years, tech_scf, 
                               string_def=paste("[", x[1,length(dimnames(x)[[2]]),1],",XXX,*,*]\t:", sep=""), 
...
last_line <- paste(last_line_raw, ";", sep="")
...
mat_pp_emissions <- matrix(power_plant_emissions_raw$emission_factor__tco2e_mwh, 
                           nrow=length(power_plants_fossil), ncol=no_years, byrow=TRUE)
...
write_matrix_to_file(mat_pp_emissions, power_plants_fossil, years, ppem_string_init, 
